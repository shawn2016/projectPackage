...
const handleChangeLoading = (res) => { // 处理显示的loading
  console.log(count)
  if ((res && res.type && res.type === 'LOGIN_LOGIN') || (res && res.type && res.type === 'SENDSMSCODE')) {
...
export default () => next => action => {
  ...
  const { isHideLoading } = callApi
  count++
  if (!isHideLoading) {
    handleChangeLoading()
  }
  return utils.callApi(callApi).then(
    res => {
      next(res)
      console.log(res)
      console.log('我是middleware')
      ...
      // }
      count--
      if (res.type === 'LOGIN_LOGIN' || res.type === 'SENDSMSCODE') {
        handleChangeLoading(res)
      } else {
        handleChangeLoading()
      }
      return res
