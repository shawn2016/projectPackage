...
class Info extends PureComponent {
  ...
  _goToDetailsPage = _id => {
    this.props.history.push({
      pathname: '/grossSettlement/singleExamineDetails',
      state: { id: _id, from: 'examineSingleHandle', source: 2 }
    })
  }
  // checkbox选择
  checkedFunc = (node) => {
        ...
        <div className="">
          <div className="rob-row ">
            ...
            <div className="rob-col-lg-16 rob-col-md-16 rob-col-sm-16 rob-col-xs-24 text-left">
              <label className="rob-input-label">{this.state.ids.length}</label>
            </div>
          </div>
          <div className="rob-row">
            <div className="rob-col-lg-8 rob-col-md-8 rob-col-sm-8 rob-col-xs-24 text-right">
  ...
  alertClose = (type) => {
    if (type === 'goFailDetail') {
      sessionStorage.setItem('failureDataList', JSON.stringify({ list: this.state.detail, type: 'Approve', from: 'single' }))
                    ...
                    <TableBody>
                      {
                        this.state.data ? this.state.data.map((item, index) => (
                          <TableRow key={`info${index + 1}`}>
                            ...
                            <TableRowColumn> {Filter.timestampFormat(item.createTime, 3, true)}</TableRowColumn>
                            ...
                            <TableRowColumn> {Filter.formatMoneyYuan(item.amount, 2)}</TableRowColumn>
...
export default connect(state => ({
  isSearch: state.GSESH_batchHandleExamineInfo && state.GSESH_batchHandleExamineInfo.GSESH_isSearch,
  queryInfo: state.GSESH_batchHandleExamineInfo && state.GSESH_batchHandleExamineInfo.GSESH_queryInfo,
  orderList: state.GSESH_batchHandleExamineInfo && state.GSESH_batchHandleExamineInfo.GSESH_orderList,
  pageInfo: state.GSESH_batchHandleExamineInfo && state.GSESH_batchHandleExamineInfo.GSESH_pageInfo
}), dispatch => ({
  isSearchFunc: bindActionCreators(actions.GSESH_isBatchExamineSearch, dispatch),
