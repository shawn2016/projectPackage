...
class Info extends PureComponent {
   ...
   */
  _cancelItem = (type = {}) => {
    if (!type.id && this.state.ids.length === 0) {
      return false
    }
    this.setState({
  ...
  _goToDetailsPage = _id => {
    this.props.history.push({
      pathname: '/grossSettlement/singleCancelDetails',
      state: { id: _id, from: 'cancelSingleHandle' }
    })
  }
  /* 关闭弹出事件 */
  alertClose = (type) => {
    ...
    if (type === 'goFailDetail') {
      sessionStorage.setItem('failureDataList', JSON.stringify({ list: this.state.detail, type: 'Revoke', from: 'single' }))
...
export default connect(state => ({
  ...
  orderList: state.GSCSH_batchHandleCancelInfo && state.GSCSH_batchHandleCancelInfo.GSCSH_orderList,
  pageInfo: state.GSCSH_batchHandleCancelInfo && state.GSCSH_batchHandleCancelInfo.GSCSH_pageInfo,
  examineResult: state.GSCSH_batchHandleCancelInfo && state.GSCSH_batchHandleCancelInfo.GSCSH_examineResult
}), dispatch => ({
