...
class StepThree extends PureComponent {
  ...
  */
  ...
  componentWillReceiveProps(nextProps) {
    const { smallRegisterCompanyInfo: finishedData = {} } = nextProps
    this.setState({
      isShow: true
    })
  ...
  render() {
    ...
    return (
      <div>
        {isShow ?
          <div className="rob-container lightGallery" style={{ display: stepNum === 3 ? 'block' : 'none' }}>
...
export default connect(state => ({
  smallRegisterCompanyInfo: state.smallRegisterReducer && state.smallRegisterReducer.registerSmallSaveInfo
}), () => ({
