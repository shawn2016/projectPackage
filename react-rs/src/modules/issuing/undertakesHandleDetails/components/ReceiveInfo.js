...
import { formatMoneyYuan, formatDate, grossReceiverType } from 'utils/filterCommon'
    ...
    this.state = {
      conf: {
        ...
        onChange: (currentPage) => {
          ...
          let param = Object.assign({}, this.props.queryInfo, { page: currentPage }, { serialNo: this.props.id }, { rows: 10 })
                ...
                <div className="rob-table-responsive">
                  <Table striped hoverEffect checkboxType={'default'}>
                    <TableHeader>
                      ...
                      <TableHeaderColumn> 收款方姓名</TableHeaderColumn>
                      ...
                      {this.props.params.from === 'undertakesHandleSearch' ? <TableHeaderColumn> 处理时间</TableHeaderColumn> : null}
                      {this.props.params.from === 'undertakesHandleSearch' ? <TableHeaderColumn style={{ height: '100px' }}>
                        <QBSelect
                          label=""
                          type="default"
                      ...
                      {this.props.params.from === 'undertakesHandleSearch' ? <TableHeaderColumn> 备注</TableHeaderColumn> : null}
                    </TableHeader>
                    <TableBody>
                      {
                        mapArray ? mapArray.map((item, index) => (
                          <TableRow key={`info${index + 1}`}>
                            ...
                            {this.props.params.from === 'undertakesHandleSearch' ? <TableRowColumn> {formatDate(item.handleTime)}</TableRowColumn> : null}
                            {this.props.params.from === 'undertakesHandleSearch' ? <TableRowColumn> {grossReceiverType(item.status)}</TableRowColumn> : null}
                            {this.props.params.from === 'undertakesHandleSearch' ? <TableRowColumn> {item.remark}</TableRowColumn> : null}
                          </TableRow>
                        )) : null
                      }
                    </TableBody>
...
export default connect(state => ({
  ...
  resetCurrentPage: bindActionCreators(actions.IUTHD_resetCurrentPage, dispatch),
}))(ReceiveInfo)