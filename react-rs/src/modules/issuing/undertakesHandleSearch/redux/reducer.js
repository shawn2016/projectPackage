...
/* 列表*/
const IUTH_orderList = (state = { values: [], pagenation: { itemCount: 0 } }, action) => {
  switch (action.type) {
    case IUTH_GETORDERLIST:
...
/* 查询条件*/
const IUTH_queryInfo = (state = { page: 1, rows: 10 }, action) => {
  switch (action.type) {
    case IUTH_UPDATESEARCHINFO:
      return Object.assign({}, state, action.value)
    default:
      return state
  }
}
const IUTH_isReserCurrentPage = (state = 0, action) => {
  switch (action.type) {
    case 'IUTH_RESETCURRENTPAGE':
      return state === 0 ? 1 : 0
    default:
      return state
  }
...
const IUTH_companyAccList = (state = [], action) => {
  switch (action.type) {
    case IUTH_GETCOMPANYACCLIST:
...
/* 状态列表*/
const IUTH_statusList = (state = _statusList, action) => {
  switch (action.type) {
    case IUTH_GETSTATUSLIST:
      return state
    default:
      return state
  }
...
const searchsingleInfo = combineReducers({ IUTH_orderList, IUTH_queryInfo, IUTH_isReserCurrentPage, IUTH_companyAccList, IUTH_statusList })
injectReducer(searchsingleInfo, 'IUTH_singleHandleSearchInfo')