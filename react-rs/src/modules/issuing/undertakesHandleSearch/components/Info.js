...
class Info extends PureComponent {
                    ...
                    <TableBody>
                      {
                        mapArray ? mapArray.map((item, index) => (
                          <TableRow key={`info${index + 1}`}>
                            ...
                            <TableRowColumn title={item.payerAccountName + item.payerAccountNo}> {item.payerAccountName}{item.payerAccountNo}</TableRowColumn>
                            ...
                            <TableRowColumn> {formatMoneyYuan(item.successAmount)}</TableRowColumn>
  ...
  _goToDetailsPage = item => {
    this.props.history.push({
      pathname: '/issuing/undertakesSearchDetails',
      state: { data: item, from: 'undertakesHandleSearch' }
    })
  }
}
...
export default connect(state => ({
  dataObj: state.IUTH_singleHandleSearchInfo && state.IUTH_singleHandleSearchInfo.IUTH_orderList,
  queryInfo: state.IUTH_singleHandleSearchInfo && state.IUTH_singleHandleSearchInfo.IUTH_queryInfo,
  isReserCurrentPage: state.IUTH_singleHandleSearchInfo && state.IUTH_singleHandleSearchInfo.IUTH_isReserCurrentPage
}), dispatch => ({
  getOrderList: bindActionCreators(actions.IUTH_getOrderList, dispatch),
  reserCurrentPage: bindActionCreators(actions.IUTH_resetCurrentPage, dispatch),
}))(Info)