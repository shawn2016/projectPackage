...
class Info extends PureComponent {
  ...
  _goToDetailsPage = item => {
    this.props.history.push({
      pathname: '/issuing/undertakesCancelDetails',
      state: { data: item, from: 'undertakesHandleCancel' }
    })
  }
  /* 关闭弹出事件 */
  alertClose = (type) => {
    ...
    if (type === 'goFailDetail') {
      sessionStorage.setItem('failureDataList', JSON.stringify({ list: this.state.detail, type: 'Revoke', from: 'agency' }))
  ...
  _save = (type) => {
    ...
    (async () => {
      let data = await getRequest({
        path: '/payment/agency/revokeOrder',
        method: 'POST',
        param: {
  ...
  checkedFunc = (node) => {
    ...
    if (node) {
      ...
      if (node.isCheck && this.state.ids.length === data.length) {
                    ...
                    <TableBody>
                      {
                        this.state.data ? this.state.data.map((item, index) => (
                          <TableRow key={`info${index + 1}`}>
                            ...
                            <TableRowColumn> {item.payerAccountName}</TableRowColumn>
...
export default connect(state => ({
  queryInfo: state.undertakesHandleCancelInfo && state.undertakesHandleCancelInfo.queryInfo,
  orderList: state.undertakesHandleCancelInfo && state.undertakesHandleCancelInfo.orderList,
  pageInfo: state.undertakesHandleCancelInfo && state.undertakesHandleCancelInfo.pageInfo,
  examineResult: state.undertakesHandleCancelInfo && state.undertakesHandleCancelInfo.examineResult
}), dispatch => ({
  updateSearchInfo: bindActionCreators(actions.updateUndertakesCancelSearchInfo, dispatch),
  getOrderList: bindActionCreators(actions.getUndertakesCancelOrderList, dispatch)
}))(Info)